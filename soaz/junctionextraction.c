...
int filled,**mask,**maskdiv,**draindir,**intern,*avcount,*flatis,*flatjs,*avcount2,*avcount3,*avcount4,ind,numbins,threshmaskarea,threshfilled;
unsigned long count,*topovecind,*iup,*idown,*jup,*jdown,lattice_size_x,lattice_size_y,ic,jc,countermask;
float max,threshdrop,threshslope,threshmfdtod8,maxarea,thresharea,threshhillslopecontrib,oneoverdeltax,elevc,*avangle,*avangle2,*avangle3,*avslope,**angle,*flatelevs,**areamfd,**slopex,**slopey,**topoold,**topo,**slope,**area,*topovec,flow1,flow2,flow3,flow4,flow5,flow6,flow7,flow8,deltaxinkm,deltax;
...
void setupgrids()
{
	 ...
	 areamfd=matrix(1,lattice_size_x,1,lattice_size_y);
	 mask=imatrix(1,lattice_size_x,1,lattice_size_y);
	 maskdiv=imatrix(1,lattice_size_x,1,lattice_size_y);
	 ...
	 avcount3=ivector(1,numbins);
	 avcount4=ivector(1,numbins);
	 avangle=vector(1,numbins);
	 avangle2=vector(1,numbins);
	 avangle3=vector(1,numbins);
	 avslope=vector(1,numbins);
	 flatelevs=vector(1,stacksize);
	   ...
	   {filled=0;
	    ...
	    while ((count>0)&&(filled<threshfilled))
		 {pop();
          ...
          if ((topo[ic][jc]>0)&&(topo[ic][jc]<=max)&&(count<stacksize))
	   ...
	   {filled=0;
	    ...
	    while ((count>0)&&(filled<threshfilled))
		 {pop();
          ...
          if (topo[idown[ic]][jup[jc]]<max) max=topo[idown[ic]][jup[jc]];
          if (topo[iup[ic]][jdown[jc]]<max) max=topo[iup[ic]][jdown[jc]];
          if ((topo[ic][jc]>0)&&(intern[ic][jc]==1)&&(topo[ic][jc]<=max)&&(count<stacksize))
		   {topo[ic][jc]=max+fillincrement;
		    if (topo[ic][jc]-topoold[ic][jc]>filled) filled=topo[ic][jc]-topoold[ic][jc];
		    push(ic,jc);
			push(iup[ic],jc);
...
{    float tot;
     ...
     areamfd[iup[i]][j]+=areamfd[i][j]*flow1;
     areamfd[idown[i]][j]+=areamfd[i][j]*flow2;
     areamfd[i][jup[j]]+=areamfd[i][j]*flow3;
     areamfd[i][jdown[j]]+=areamfd[i][j]*flow4;
     areamfd[iup[i]][jup[j]]+=areamfd[i][j]*flow5;
     areamfd[iup[i]][jdown[j]]+=areamfd[i][j]*flow6;
     areamfd[idown[i]][jup[j]]+=areamfd[i][j]*flow7;
     areamfd[idown[i]][jdown[j]]+=areamfd[i][j]*flow8;
}
	...
	for (j=1;j<=lattice_size_y;j++)
     for (i=1;i<=lattice_size_x;i++)
	  {calculatedrainagedirections(i,j);
	   ...
	   areamfd[i][j]=deltaxinkm*deltaxinkm;}
	...
	for (j=1;j<=lattice_size_y;j++)
     for (i=1;i<=lattice_size_x;i++)
	  {slopex[i][j]/=area[i][j]/(deltaxinkm*deltaxinkm);
	   ...
	   if ((areamfd[i][j]/area[i][j]>threshmfdtod8)&&(slopel<threshslope)) maskdiv[i][j]=1; 
	   if ((area[i][j]<thresharea)||(maskdiv[i][j]==1)) area[i][j]=0;}
}
...
{   unsigned long i,j,n,ilast,jlast,il,jl,i1,j1,i2,j2,i3,j3,i2k,j2k,i1k,j1k;
    ...
    fp0=fopen("./soazjunctions.txt","w");	
	for (j=1;j<=lattice_size_y;j++)
     for (i=1;i<=lattice_size_x;i++)
	  if (mask[i][j]==1)
	   {//identify pixel in direction of upstream trib 1
        max=0;
        if ((area[iup[i]][j]>max)&&(area[i][j]>area[iup[i]][j])) {max=area[iup[i]][j];i1=iup[i];j1=j;}
        if ((area[idown[i]][j]>max)&&(area[i][j]>area[idown[i]][j])) {max=area[idown[i]][j];i1=idown[i];j1=j;}
		if ((area[i][jup[j]]>max)&&(area[i][j]>area[i][jup[j]])) {max=area[i][jup[j]];i1=i;j1=jup[j];}
        ...
        i2k=i2;j2k=j2;  
		//identify pixel in direction of downstream channel 3
		i3=i;j3=j;
		dist=0;
		max=0;
		...
		//search downslope along main channel 3 to compute slope and direction
		...
		//search upslope along tributary 1 to compute slope and direction
		dist=0;
	    i1=i1k;j1=j1k;
...
int main()
{
	 ...
	 threshdrop=10;                // m
	 threshfilled=10;              // m
	 threshslope=0.03;             // m/m
     threshmfdtod8=10;	 
	 numbins=30;
	 setupgridneighbors();
	 setupgrids();
	 fp0=fopen("./soaz.txt","r");
     for (j=1;j<=lattice_size_y;j++)
	  for (i=1;i<=lattice_size_x;i++)
	 ...
	 fp0=fopen("./soazmeanjunctions.txt","w");
